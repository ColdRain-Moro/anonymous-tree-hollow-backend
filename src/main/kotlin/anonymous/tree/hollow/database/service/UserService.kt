...
class UserService {
    ...
    fun login(email: String, password: String): Boolean {
        return transaction {
            val result = UserEntity.find { userTable.email eq email }.firstOrNull() ?: return@transaction false
            (password +  result.salt).md5() == result.md5Password
        }
    }

    fun register(email: String, password: String): Boolean {
        return transaction {
            if (!UserEntity.find { userTable.email eq email }.empty()) return@transaction false
            UserEntity.new {
                this.email = email
